...
public class DesafioControllerIntegrationTests {
    ...
    public void testGetAccountBalance() throws Exception {
        ...
        mockMvc.perform(get("/desafio/api/v1/accountBalance/" + accountId))
                .andExpect(status().isOk())
    ...
    public void testGetAllTransactions() throws Exception {
        ...
        mockMvc.perform(get("/desafio/api/v1/transactions/" + accountId))
                .andExpect(status().isOk())
    ...
    public void testGetTransactionsByDateRange() throws Exception {
        ...
        mockMvc.perform(get("/desafio/api/v1/transactionsByRange")
    ...
    @Test
    @Order(9)
    public void testGetImprovements() throws Exception {
        // Perform request to get improvements
        mockMvc.perform(get("/desafio/api/v1/improvements"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.['Qualidade e Organização do Código']").isArray())
                .andExpect(jsonPath("$.['Performance']").isArray())
                .andExpect(jsonPath("$.['Segurança']").isArray())
                .andExpect(jsonPath("$.['Documentação']").isArray())
                .andExpect(jsonPath("$.['Testes']").isArray())
                .andExpect(jsonPath("$.['Design de API']").isArray())
                .andExpect(jsonPath("$.['Qualidade e Organização do Código'][0]").isString())
                .andExpect(jsonPath("$.['Qualidade e Organização do Código']", hasSize(5)))
                .andExpect(jsonPath("$.['Performance']", hasSize(3)))
                .andExpect(jsonPath("$.['Segurança']", hasSize(5)))
                .andExpect(jsonPath("$.['Documentação']", hasSize(3)))
                .andExpect(jsonPath("$.['Testes']", hasSize(3)))
                .andExpect(jsonPath("$.['Design de API']", hasSize(5)));
    }
}
